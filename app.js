    ...
    if (!error) {
      ...
      faucetBalance().then(balance => {
        if (balance) {
          ...
          client.post('statuses/update', { status: 'Faucetの残高は: ' + balance + ' xem です。' }, function (error, tweet, response) {
            if (!error) {
...
new CronJob('00 00 * * * *', function () {
  ...
  client.post('statuses/update', { status: regulartweet }, function (error, tweet, response) {
    if (!error) {
